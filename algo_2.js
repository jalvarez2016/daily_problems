let r = `4-8 n: dnjjrtclnzdnghnbnn
5-6 r: rrrrcqr
2-4 p: pqhpppwpfmr
...
12-13 r: rrrpjrrrrrrtfrkwmr`.split("\n");
// console.log(r);
let num = r.reduce((acc, val)=> {
    let range = {
        min : Number(val.split(" ")[0].split("-")[0]),
        max : Number(val.split(" ")[0].split("-")[1])
    }
    let searchChar = val[val.split("").findIndex(x => x ===":") - 1];
    const regex = new RegExp(searchChar, "g");
    const searchStr = val.split(" ")[2];
    let numsFound = (searchStr.match(regex) || []).length;
    // console.log(acc);
    if(numsFound <= range.max && numsFound >= range.min){
        return acc+1;
    } else {
        return acc;
    }
...
const num2 = r.reduce((acc, val) => {
    const places = {
        first : Number(val.split(" ")[0].split("-")[0]),
        second : Number(val.split(" ")[0].split("-")[1])
    }
    const searchChar = val[val.split("").findIndex(x => x === ":") -1];
    const searchStr = val.split(" ")[2];
    console.log(searchChar, searchStr[places.first - 1], searchStr[places.second - 1], acc);
    if(searchChar === searchStr[places.first - 1] && searchStr[places.second - 1] !== searchChar){
        // console.log(acc);
        return acc + 1;
    } else if (searchChar === searchStr[places.second - 1] && searchStr[places.first - 1] !== searchChar) {
        return acc + 1;
    }
    // console.log(false);
    return acc;
