...
byr:2008`.split("\n\n");
const keys = `ecl pid eyr hcl byr iyr hgt`.split(" ");
let validObj = (obj) => {
    return keys.every(val => {
        // console.log(val in obj, val, obj);
        if ( val in obj  ) {
            if(val === "byr" && obj[val] >= 1920 && obj[val] <= 2002 && obj[val].length  === 4){
                return true;
            } else if (val === "iyr" && obj[val].length === 4 && obj[val] >= 2010 && obj[val] <= 2020) {
                return true;
            } else if (val === "eyr" && obj[val].length === 4 && obj[val] >=2020 && obj[val] <= 2030) {
                return true;
            } else if(val === "hgt") {
                if (obj[val].findIndex("cm") > -1) {
                    if (obj[val].split("cm")[0] >= 150 && obj[val].split("cm")[0] <= 193) {
                        return true;
                    }
                } else if (obj[val].findIndex("in") > -1) {
                    if (obj[val].split("in")[0] >= 59 && obj[val].split("in")[0] <= 76) {
                        return true;
                    }
                }
            } else if (val === "hcl") {
                let re = new RegExp(/[abcdef0-9]/, "g");
                if(obj[val].match(re).length === 6) {
                    return true;
                }
            } else if(val === "ecl") {
